...
export function GovPlanAuditPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (audit) => {
      showToast({
        type: 'success',
        title: canAssignAuditor ? 'Audit planned and assigned' : 'Audit planned',
      });
  ...
  return (
    <div className="grid gap-5">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <article className="rounded-2xl border border-brand-100 bg-brand-50/60 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-700">
              Start with a school inside the active scope. If a school does not appear, it is
              outside your current area, inactive, or your assignment window is not active yet.
            </p>
          </article>
          <article className="rounded-2xl border border-brand-100 bg-white p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-700">
              Select the audit type, planned date, and notes so the visit has a clear purpose before
              field work starts.
            </p>
          </article>
          <article className="rounded-2xl border border-brand-100 bg-white p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-700">
              Planning creates the audit record. After the visit, the assigned auditor submits the
              scored report and recommendations from the audit details page.
            </p>
          </article>
        </div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="rounded-2xl border border-brand-100 bg-white p-4">
            ...
            <div className="mt-3 grid gap-2 text-sm leading-6 text-slate-700">
              <p>
                Planning links the audit to the correct school, scope, and date before any report is
                submitted.
              </p>
              <p>
                It gives the auditor a clear mission, helps supervisors track pending visits, and
                keeps the audit trail complete.
              </p>
              <p>
                Without a planned audit, there is no valid record to conduct, score, or report
                against later.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
            ...
            <div className="mt-3 grid gap-2 text-sm leading-6 text-amber-900">
              <p>
                You can act only inside schools that match your active country, province, district,
                or sector assignment.
              </p>
              <p>
                Assignment start and end dates also matter. If the assignment window is inactive,
                schools and actions will disappear.
              </p>
              <p>
                Government auditors can plan audits, submit audit reports, and leave incident
                feedback. School disciplinary actions stay read-only for oversight users.
              </p>
            </div>
          </div>
        </div>
      ...
      {!canAssignAuditor && myScopes.length ? (
        <SectionCard
          title="Active Scope"
          subtitle="These assignments determine where you can plan or conduct audits."
        >
          <div className="grid gap-3">
            {myScopes.map((scope) => (
              <article
                key={scope.id}
                className="rounded-2xl border border-brand-100 bg-white p-4 text-sm text-slate-700"
              >
      ...
      <SectionCard
        title="Audit Form"
        subtitle="Use the shared audit model so planning stays consistent across auditor workflows."
      >
        <form
              ...
              <label className="grid gap-1 text-sm text-slate-700 md:col-span-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setForm((current: GovPlanAuditFormValue) => ({
                      ...current,
                      auditorUserId: event.target.value,
                    }))
                  }
                  className="h-10 rounded-lg border border-brand-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-brand-400"
                  required
                ...
                >
                  <option value="">
                    {auditorsQuery.isPending ? 'Loading auditors...' : 'Select auditor'}
                  </option>
          ...
          {canAssignAuditor ? (
            eligibleAuditors.length === 0 ? (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                No active auditors with assignments are available yet. Create an auditor or add an
                assignment before planning a school audit.
              </div>
            ) : selectedAuditor ? (
              <div className="grid gap-4 rounded-2xl border border-brand-100 bg-brand-50/50 p-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-slate-700">
                    {selectedAuditor.phone || 'No phone number saved'}
                  </p>
                ...
                <div className="space-y-2">
                  ...
                  {activeScope?.assignedBy ? (
                    <p className="text-sm text-slate-700">
                      Assigned by {activeScope.assignedBy.firstName}{' '}
                      {activeScope.assignedBy.lastName}
                    </p>
                  ) : null}
                  ...
                  {activeScope?.notes ? (
                    <p className="text-sm text-slate-700">{activeScope.notes}</p>
                  ) : null}
                </div>
              </div>
            ) : null
