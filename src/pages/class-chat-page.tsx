...
function renderContentWithMentions(content: string, mentionedUserIds: string[], roster: ChatMessage['sender'][]) {
  if (!mentionedUserIds.length) return content;
  const names = mentionedUserIds
    .map(id => (roster as Array<{ id: string; firstName: string; lastName: string }>).find(r => r.id === id))
...
export function ClassChatPage() {
  ...
  const chatQuery = useQuery({
    ...
    queryFn: () => getOrCreateChatApi(auth.accessToken!, classRoomId!, globalAcademicYearId ?? undefined),
  ...
  const messagesQuery = useQuery({
    ...
    queryFn: () => listChatMessagesApi(auth.accessToken!, chatId!, { pageSize: 100, q: debouncedSearch || undefined }),
  ...
  useEffect(() => {
    ...
    function upsertMessage(msg: ChatMessage, append: boolean) {
      queryClient.setQueryData<ChatMessagesResponse | undefined>(['chat-messages', chatId, debouncedSearch], old => {
        if (!old) return old;
    ...
    function handlePresence({ chatId: cid, onlineUserIds: ids }: { chatId: string; onlineUserIds: string[] }) {
      if (cid === chatId) setOnlineUserIds(ids);
    }
    function handleTyping({ chatId: cid, userId: uid, userName }: { chatId: string; userId: string; userName?: string }) {
  ...
  const reactMutation = useMutation({
    mutationFn: ({ messageId, emoji, alreadyReacted }: { messageId: string; emoji: string; alreadyReacted: boolean }) =>
      alreadyReacted
        ? removeReactionApi(auth.accessToken!, chatId!, messageId, emoji)
        : reactToMessageApi(auth.accessToken!, chatId!, messageId, emoji),
    onError: (e) => showToast({ type: 'error', title: 'Could not react', message: (e as ApiClientError).message }),
  });

  const pinMutation = useMutation({
    mutationFn: ({ messageId, pinned }: { messageId: string; pinned: boolean }) =>
      pinned ? unpinMessageApi(auth.accessToken!, chatId!, messageId) : pinMessageApi(auth.accessToken!, chatId!, messageId),
    onError: (e) => showToast({ type: 'error', title: 'Could not update pin', message: (e as ApiClientError).message }),
  });
  ...
  if (chatQuery.isError) {
    return (
      <StateView
        ...
        message={(chatQuery.error as ApiClientError)?.message ?? 'You may not have access to this class chat.'}
      />
    );
  }
  ...
  const mentionMatches =
    mentionQuery !== null
      ? roster.filter(r => r.firstName.toLowerCase().startsWith(mentionQuery.toLowerCase())).slice(0, 5)
      : [];
  ...
  return (
    <div className="flex h-[calc(100vh-10rem)] flex-col rounded-2xl border border-slate-200 bg-white shadow-soft">
      <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-5 py-3">
        <div>
          <h2 className="text-lg font-bold text-slate-900">{chatQuery.data?.classRoom.name ?? 'Class Chat'}</h2>
        ...
        {messagesQuery.isPending ? (
          ...
          messagesQuery.data?.items.map((msg) => {
            ...
            const readByOthers = (readReceiptsQuery.data ?? []).filter(
              r => r.user.id !== msg.sender.id && new Date(r.lastReadAt) >= new Date(msg.createdAt)
            );
            return (
              <div key={msg.id} className={`group flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] rounded-xl px-4 py-2 ${isMine ? 'bg-brand-500 text-white' : 'bg-slate-100 text-slate-900'}`}>
                    ...
                    {!msg.isDeleted && (
                      <div className="flex items-center gap-1 opacity-0 transition group-hover:opacity-100">
                        <button
                          ...
                          className={isMine ? 'text-white/80 hover:text-white' : 'text-slate-500 hover:text-slate-800'}
                        >
                        ...
                        {permissions?.canPin && (
                          <button
                            ...
                            onClick={() => pinMutation.mutate({ messageId: msg.id, pinned: msg.isPinned })}
                            className={isMine ? 'text-white/80 hover:text-white' : 'text-slate-500 hover:text-slate-800'}
                          >
                        ...
                        {permissions?.canModerate && (
                          <button
                            ...
                            className={isMine ? 'text-white/80 hover:text-white' : 'text-slate-500 hover:text-red-600'}
                          >
                  ...
                  {msg.isPinned && <p className="mt-0.5 text-[10px] font-semibold uppercase opacity-70">📌 Pinned</p>}

                  {msg.replyTo && (
                    <div className={`mt-1 rounded-lg border-l-2 px-2 py-1 text-xs opacity-80 ${isMine ? 'border-white/40' : 'border-slate-300'}`}>
                    ...
                    <>
                      {msg.content && (
                        <p className="mt-1 whitespace-pre-wrap text-sm">
                          {renderContentWithMentions(msg.content, msg.mentionedUserIds, roster as never)}
                        </p>
                      )}
                      {msg.attachment && (
                        <div className="mt-2">
                          {msg.attachment.mimeType === 'application/pdf' ? (
                            <SecurePdfViewer assetId={msg.attachment.id} accessToken={auth.accessToken!} className="max-w-xs" />
                          ) : msg.attachment.resourceType === 'IMAGE' && msg.attachment.secureUrl ? (
                            <img
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {msg.reactions.map((r) => (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            reactMutation.mutate({ messageId: msg.id, emoji: r.emoji, alreadyReacted: r.reactedByMe })
                          }
                          className={`rounded-full border px-1.5 py-0.5 text-[11px] ${
                            r.reactedByMe ? 'border-brand-400 bg-brand-50 text-brand-700' : 'border-slate-200 bg-white/70 text-slate-600'
                          }`}
                        >
                          {r.emoji} {r.count}
                        </button>
                    ...
                    <div className="mt-1.5 flex gap-1 rounded-lg border border-slate-200 bg-white p-1 shadow-sm">
                      {QUICK_REACTIONS.map((emoji) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            const already = msg.reactions.find(r => r.emoji === emoji)?.reactedByMe ?? false;
                            reactMutation.mutate({ messageId: msg.id, emoji, alreadyReacted: already });
        ...
        <div className="border-t border-slate-200 px-5 py-3">
          ...
          <div className="relative">
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                if ((message.trim() || attachmentFile) && !sendMutation.isPending) sendMutation.mutate();
              }}
            ...
            >
              ...
              >
                {sendMutation.isPending || isUploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                Send
              </button>
            </form>
          </div>
